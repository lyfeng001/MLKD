...
def draw_success_precision(success_ret, name, videos, attr, precision_ret=None,
    ...
    if attr == 'ALL':
        plt.title(r'\textbf{Success plots on %s}' % (name))
    else:
    ...
    plt.show()
    plt.savefig('/home/user/V4R/LYF/pysot-master2/toolkit/visualization/output/success.png', dpi=900, bbox_inches='tight')
    ...
    if precision_ret:
        ...
        if attr == 'ALL':
            plt.title(r'\textbf{Precision plots on %s}' % (name))
        else:
        ...
        ax.set_aspect((xmax - xmin)/(ymax-ymin))
        plt.show()
        plt.savefig('/home/user/V4R/LYF/pysot-master2/toolkit/visualization/output/precision.png', dpi=900, bbox_inches='tight')
    # norm precision plot
