...
class SubDataset(object):
    def __init__(self, name, root, anno, frame_range, num_use, start_idx):
        ...
        for video in list(meta_data.keys()):
            for track in meta_data[video]:
                ...
                frames.sort()
                meta_data[video][track]['frames'] = frames #int(track)
                if len(frames) <= 0:
    ...
    def get_image_anno(self, video, track, frame):
        ...
        frame1 = "{:02d}".format(fff)
        track1 = '000000'
        image_path = os.path.join(self.root, video,
                                  self.path_format.format(track1, frame1, 'x'))
...
class TrkDataset(Dataset):
    def __init__(self,):
        ...
        if desired_size != cfg.TRAIN.OUTPUT_SIZE:
            raise Exception('size not match!')          #判断能否产生正确的输出
        ...
        for name in cfg.DATASET.NAMES:
            ...
            self.num += sub_dataset.num_use             #将不同的几个数据集分别构建
